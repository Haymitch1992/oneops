...
export default class CForm extends Vue {
  ...
  private reset() {
    ...
    for (const p in this.formProvide.formObj) {
      if (Array.isArray(this.defaultFormObj[p])) {
        this.formProvide.formObj[p] = this.defaultFormObj[p].map((item: string | {}, index: number) => {
          if (typeof item === 'string') {
            ...
            // immutable
            const _obj: any = {}
            for (const _p in this.defaultFormObj[p][index]) {
